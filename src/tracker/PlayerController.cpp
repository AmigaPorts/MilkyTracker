...
private:
							...
							mp_sint32 rangeStart, mp_sint32 rangeEnd)
	{
		...
		if (rangeStart == -1 && rangeEnd == -1)
		{
			...
			if (flags & 3)
				player.playSample(chn, smp->sample, smp->sample2x, smp->sample4x, smp->sample8x, smp->samplen, 0, 0, false, smp->loopstart, smp->loopstart+smp->looplen, flags);
			else
				player.playSample(chn, smp->sample, smp->sample2x, smp->sample4x, smp->sample8x, smp->samplen, 0, 0, false, 0, smp->samplen, flags);
		}
		else
		{
			if (rangeStart == -1 || rangeEnd == -1)
			...
			player.playSample(chn, smp->sample, smp->sample2x, smp->sample4x, smp->sample8x, smp->samplen, rangeStart, 0, false, 0, rangeEnd, flags);
		}
	}
